...
namespace LJH.GeneralLibrary.SoftDog
{
    public class SoftDogReader
    {
        ...
        private string ReadString(int len, int addr)
        {
            ...
            if (ReadDog_Str(len, addr, strTemp, lngVerify) == 0)
            {
        ...
        public SoftDogInfo ReadDog()
        {
            ...
            try
            {
                if (CheckDog())
                {
                    ...
                    info.TryPeriodStartDate = DateTime.Parse("20" + d.Substring(0, 2) + "-" + d.Substring(2, 2) + "-" + d.Substring(4, 2));
                    info.TryPeriodDays1 = ReadInteger(2, 77);
                    info.TryPeriodDays2 = ReadInteger(2, 81);
                }
            }
            catch (Exception ex)
            {
